    ...
    <form method="POST" action="{{ route('componentes.update', $componente->id_componente) }}">
        ...
        <div id="tarjeta_madre_campos" style="display:none;">
            ...
            <div class="form-group">
                ...
                <div>
                    ...
                    $seleccionadosInternos = array_map('trim', explode(',', $valorAnterior)); // <- trim aquí
                        }
                        @endphp
        ...
        <div class="form-group">
            ...
            <div>
                ...
                $otrosConectores[] = $v;
                unset($seleccionadosCon[$k]); // eliminar de la lista normal
                }
                }
...
<div id="tarjeta_grafica_campos" style="display:none;">
    ...
    <div class="form-group">
        ...
        @php
        // Obtener el valor actual de la BD o lo que venga de "old" (si hubo error de validación)
        $salidasSeleccionadas = old('salidas_video', $componente->salidas_video ?? '');

        // Convertir a array si viene como string tipo "HDMI, VGA"
        ...
        <label>
            <input type="checkbox" name="salidas_video[]" value="VGA"
                {{ in_array('VGA', $salidasSeleccionadas ?? []) ? 'checked' : '' }}>
            VGA
        </label><br>

        <label>
            <input type="checkbox" name="salidas_video[]" value="HDMI"
                {{ in_array('HDMI', $salidasSeleccionadas ?? []) ? 'checked' : '' }}>
            HDMI
        </label><br>

        <label>
            <input type="checkbox" name="salidas_video[]" value="DVI"
                {{ in_array('DVI', $salidasSeleccionadas ?? []) ? 'checked' : '' }}>
            DVI
        </label><br>

        <label>
            <input type="checkbox" name="salidas_video[]" value="DisplayPort"
                {{ in_array('DisplayPort', $salidasSeleccionadas ?? []) ? 'checked' : '' }}> DisplayPort
        </label><br>
    </div>
...
<div id="fan_cooler_campos" style="display:none;">
    ...
    <div class="form-group">
        <label>Tamaño</label>
        <input type="text" name="tamano_fan" class="form-control"
            value="{{ old('tamano_fan', $componente->tamano_fan) }}">
    </div>
    <div class="form-group">
        <label>Tipo conector</label>
        <input type="text" name="tipo_conector_fan" class="form-control"
            value="{{ old('tipo_conector_fan', $componente->tipo_conector_fan) }}">
    </div>
    <div class="form-group">
        <label>Estado</label>
